...
- **ML/NER redaction** *(optional, off by default)* — adds `PERSON` and `LOCATION` detection that regex cannot catch, behind the `ner` cargo feature (`cargo build --features ner`) and the `--pii-ner` / `PII_NER_ENABLE` toggle. Inference uses **candle** (pure Rust) — no external runtime library, so the ML build is still a single binary plus a user-supplied model directory (`config.json`, `tokenizer.json`, `model.safetensors`; recommended: the MIT-licensed [`dslim/bert-base-NER`](https://huggingface.co/dslim/bert-base-NER)). Fail-closed: a model that cannot load aborts startup and an inference error fails the request — never a silent fallback. Respects `--pii-categories`. English for v1. The default build pulls no ML dependencies.
