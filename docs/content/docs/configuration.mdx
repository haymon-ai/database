...
| `--pii-ner-entity-threshold` | `PII_NER_ENTITY_THRESHOLDS` | *unset* | Per-entity confidence overrides, e.g. `ORGANIZATION=0.85,FACILITY=0.7` |
...
- **Category-aware.** The NER pass respects `--pii-categories`: `PERSON`, `ORGANIZATION`, and `NATIONALITY_RELIGION_POLITICS` are emitted only when `personal` is selected; `LOCATION` and `FACILITY` only when `contact` is. With no subset (the default), all apply. When the subset selects neither category, the model is not loaded at all.
- **Tunable per entity.** `--pii-ner-entity-threshold` / `PII_NER_ENTITY_THRESHOLDS` overrides the global confidence floor for individual entities (e.g. `ORGANIZATION=0.85`) — the recommended lever for curbing the noisier `ORGANIZATION` class without disabling it. An unknown or non-NER entity name fails closed at startup.
