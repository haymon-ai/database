...
Pass `search` (case-insensitive substring; `%` and `_` retain their `LIKE` / `ILIKE` wildcard semantics) to filter by name, or `detailed: true` to receive a JSON object whose values carry per-function metadata. Both can combine.
