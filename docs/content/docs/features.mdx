...
- **Brief** (default) — `materializedViews` is a sorted JSON array of bare matview-name strings. Wire shape is preserved from earlier releases.
...
- **Detailed** (`detailed: true`) — `materializedViews` is a JSON object keyed by bare matview name; each value carries `schema`, `owner`, `description` (or `null`), `definition`, `populated`, `indexed`. The matview name is the map key only — it is not repeated inside the value.
...
The optional `search` parameter applies a case-insensitive `ILIKE` substring filter on matview name. `%` and `_` carry their standard `ILIKE` wildcard meaning; backslash is not an escape character. SQL meta-characters (`'`, `;`, `--`) are bound as parameter values and never interpolated. `search` and `detailed` may be combined — when both are set, the detail payload is produced only for matviews that pass the filter.
