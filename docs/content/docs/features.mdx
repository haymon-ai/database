...
PostgreSQL also accepts these optional parameters:

- `search` — a case-insensitive `ILIKE` pattern filtering on view name. `%` matches any sequence of characters, `_` matches a single character. Pass `active%` for a prefix match, `%active%` for a substring match. A bare word with no wildcards matches only an exact view name. The `search` value must remain identical across paginated calls for cursor continuity.
- `detailed` — when `true`, the response includes per-view metadata (owner, comment, full SELECT body) so a single call enumerates views and inspects them. Defaults to `false`.

In **detailed mode** (PostgreSQL only) the response is keyed by **bare view name** — views cannot overload, so no signature suffix is needed:
...
Each value carries `schema` (always `"public"`), `owner` (from `pg_views.viewowner`), `description` (or `null` when no `COMMENT ON VIEW`), and `definition` (the SELECT body verbatim from `pg_views.definition` — no `CREATE VIEW` wrapper, no truncation). The view name is the map key only; it is **not** repeated inside the value. Brief and detailed pages share the same `(viewname, oid)` row order, so a client can switch `detailed` mid-pagination without losing position.

Detailed mode deliberately omits column metadata, the redundant `name` field, and view-level options (`security_barrier`, `security_invoker`, `WITH CHECK OPTION`). Column shape is recoverable from the `definition` text or via [`listTables(detailed=true)`](#listtables) — Postgres exposes views in `pg_class`, so they appear in table-style introspection too. View options are recoverable from `pg_class.reloptions` via `readQuery`.

MySQL/MariaDB and SQLite return the brief shape only — `search` and `detailed` are PostgreSQL-only for now.
