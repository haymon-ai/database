...
List user-defined triggers on tables in a database. Internal constraint and foreign-key triggers are excluded. Results are paginated — see [Cursor Pagination](#cursor-pagination) for the protocol. The response is a sorted JSON array of trigger-name strings:

```json
{
  "triggers": ["audit_orders_after_insert", "set_updated_at"],
  "nextCursor": null
}
```
