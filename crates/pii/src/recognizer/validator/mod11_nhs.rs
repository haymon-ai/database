...
#[cfg(test)]
mod tests {
    use super::Mod11NhsValidator;
    use crate::recognizer::{ValidationOutcome, Validator};

    #[test]
    fn remainder_ten_branch_is_valid() {
        // sum%11 == 10 → check = 1. Pre-fix code wrongly rejected this branch.
        assert_eq!(Mod11NhsValidator.validate("0000000051"), ValidationOutcome::Valid);
    }

    #[test]
    fn computed_check_ten_is_invalid() {
        // sum%11 == 1 → computed check = 10 → NHS spec says invalid.
        assert_eq!(Mod11NhsValidator.validate("0000003000"), ValidationOutcome::Invalid);
    }
}
