//! MCP tool registration for the MySQL/MariaDB backend.
//!
//! Each tool is registered as a [`ToolRoute`] on the [`ToolRouter`]
...
impl MysqlBackend {
    ...
    /// Registers the `read_query` tool with `MySQL` dialect validation.
