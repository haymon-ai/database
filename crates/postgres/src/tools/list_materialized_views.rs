...
/// Marker type for the `listMaterializedViews` MCP tool (pinned variant — no `database` field).
...
/// Marker type for the `listMaterializedViews` MCP tool (unpinned variant — carries `database`).
