...
<pagination>
Paginated. Pass the prior response's `nextCursor` as `cursor` to fetch the next page. The `search` filter must stay the same across pages for cursor continuity. Brief and detailed modes share the same `(viewname, oid)` row order, so a client can switch `detailed` between pages without losing position.
</pagination>"#;
}
...
/// and `v.schemaname = 'public'` keeps system-schema views out. The
/// `($1::text IS NULL OR ...)` trinary lets one statement cover both filtered
/// and unfiltered cases. `(v.viewname, c.oid)` is the sort key — `oid` is a
/// defensive tiebreaker for stable pagination (view names are unique per schema
/// by Postgres's own constraint, so it should not actually break ties, but the
/// shape stays uniform with sibling list-tools).
const BRIEF_SQL: &str = r"
    ...
    ORDER BY v.viewname, c.oid
    LIMIT $2 OFFSET $3";

/// Detailed-mode SQL: per-view `json_build_object` projection.
///
/// `pg_views` excludes materialized views. The `pg_namespace` + `pg_class`
/// joins anchor the relation OID needed by `obj_description` (and provide the
/// defensive `c.oid` tiebreaker). Postgres defers SELECT-list evaluation past
/// `LIMIT`, so `obj_description` only runs for the page's rows — never the
/// full schema. `pg_views.viewowner` is already a role name, so no `pg_roles`
/// join is needed.
